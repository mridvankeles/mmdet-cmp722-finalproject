...
class AnchorGenerator:
    ...
    def single_level_grid_priors(self,
        ...
        base_anchors = self.base_anchors[level_idx].to('cpu').to(dtype)
        feat_h, feat_w = featmap_size
        stride_w, stride_h = self.strides[level_idx]
        # First create Range with the default dtype, than convert to
        # target `dtype` for onnx exporting.
        shift_x = torch.arange(0, feat_w, device='cpu').to(dtype) * stride_w
        shift_y = torch.arange(0, feat_h, device='cpu').to(dtype) * stride_h
    ...
    def valid_flags(self, featmap_sizes, pad_shape, device='cuda'):
        ...
        for i in range(self.num_levels):
            ...
            flags = self.single_level_valid_flags((feat_h, feat_w),
                                                  ...
                                                  device='cpu')
            multi_level_flags.append(flags.to(device))
    ...
    def single_level_valid_flags(self,
        ...
        Returns:
            torch.Tensor: The valid flags of each anchor, a (H * W * num_base_anchors, )
                tensor.
        """
        feat_h, feat_w = featmap_size
        ...
        valid = valid[:, None].expand(valid.size(0),
                                      self.num_base_anchors[0]).contiguous().view(-1)
        return valid
